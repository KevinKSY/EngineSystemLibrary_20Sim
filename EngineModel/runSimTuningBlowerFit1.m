...
logVariables = {'time' ,...
    ...
    'DieselEngine2Stroke.EngineControl.EngineControl.outputEVO',...
...
for caseNo = 12:13
    ...
    %xxsimSetParameters('DieselEngine2Stroke.Constant.BSFCRef', eng_data.perf.design.BSFC(caseNo));
    %xxsimSetParameters('DieselEngine2Stroke.Constant1.QCylRef', HTRef(caseNo));
    xxsimSetParameters('DieselEngine2Stroke.BoundaryIn.T',eng_data.condition.design.T_amb+273.15);
    xxsimSetParameters('tCW',eng_data.condition.design.T_cw_scav + 273.15);
    %xxsimSetParameters('DieselEngine2Stroke.TurbochargingSystem.BlowerControl.SP',(eng_data.scvg.p_scav_g(caseNo) + 1)*1e5);
    xxsimRun();
    [values, names] = xxsimGetLogValues(logVariables);
    tempWall(caseNo-11) = mean(values(end-2000:end,2));
    pMaxCase(caseNo-11) = mean(values(end-2000:end,3));
    BSFCCase(caseNo-11) = mean(values(end-2000:end,4));
    pExhCase(caseNo-11) = mean(values(end-2000:end,5));
    TExhCase(caseNo-11) = mean(values(end-2000:end,6));
    FExhCase(caseNo-11) = mean(values(end-2000:end,7));
    pScavCase(caseNo-11) = mean(values(end-2000:end,8));
    TACoolCase(caseNo-11) = mean(values(end-2000:end,9));
    FScavCase(caseNo-11) = mean(values(end-2000:end,10));
    pACompCase(caseNo-11) = mean(values(end-2000:end,11));
    TACompCase(caseNo-11) = mean(values(end-2000:end,12));
    FACompCase(caseNo-11) = mean(values(end-2000:end,13));
    dmeCase(caseNo-11) = mean(values(end-2000:end,14));
    omegaTCCase(caseNo-11) = mean(values(end-2000:end,15));
    compEff(caseNo-11) = mean(values(end-2000:end,16));
    turbEff(caseNo-11) = mean(values(end-2000:end,17));
    dQCase(caseNo-11) = mean(values(end-2000:end,18));
    dmBlowerCase(caseNo-11) = mean(values(end-2000:end,19));
    phiInj(caseNo-11) = mean(values(end-2000:end,20));
    EVC(caseNo-11) = mean(values(end-2000:end,21));
    pCompCase(caseNo-11) = mean(values(end-2000:end,22))/1e5;
