...
pRailRef = 600:100:1500;

noCase = length(RPMRef)*length(TqRef);
...
for ii = 1:length(RPMRef)   
    for jj = 1:length(TqRef)
        caseNo = caseNo + 1;
        if BSFCCase(caseNo) == 0
            fprintf('Running case no %d, speed = %d RPM, torque = %d kN \n', caseNo, RPMRef(ii),TqRef(jj)/1000);
            xxsimSetParameters('ConstantRPMRef.RPMRef',RPMRef(ii));
            xxsimSetParameters('Constant.TqRef',TqRef(jj));


            xxsimRun();

            [values, names] = xxsimGetLogValues(logVariables);
            if abs(values(end,22)*30/pi - RPMRef(ii)) / RPMRef(ii) < 0.05
                ...
                save('SSMapResult4.mat','tempWall','pMaxCase','BSFCCase','pExhCase', ...
